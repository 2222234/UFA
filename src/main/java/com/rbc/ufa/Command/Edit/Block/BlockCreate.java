...
public class BlockCreate extends UFACommandToDo {
    private final MapManager mapManager;

    public BlockCreate(UFACommandToDo superCmd, ModeManager modeManager, Message message) {
    ...
    public void toDo(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 2) {
            try {
                ...
                if (map.getBlocks().createBlock(map.getSecLoc1(), score, repTime)) {
                    sender.sendMessage(map.getBlocks().infoLast());
                } else {
