...
public class AreaRemove extends UFACommandToDo {
    private final MapManager mapManager;

    public AreaRemove(UFACommandToDo superCmd, ModeManager modeManager, Message message) {
    ...
    public void toDo(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            try {
                ...
                if (map.getAreas().removeArea(num)) {
                    sender.sendMessage(message.getMap("message.area_remove")
