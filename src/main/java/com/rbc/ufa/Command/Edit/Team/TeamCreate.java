...
public class TeamCreate extends UFACommandToDo {
    private final MapManager mapManager;

    public TeamCreate(UFACommandToDo superCmd, ModeManager modeManager, Message message) {
    ...
    public void toDo(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            try {
                map.getTeams().createTeam();
                sender.sendMessage(map.getTeams().infoLast());
