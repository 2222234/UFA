...
public class AreaCreate extends UFACommandToDo {
    private final MapManager mapManager;

    public AreaCreate(UFACommandToDo superCmd, ModeManager modeManager, Message message) {
    ...
    public void toDo(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            try {
                int repTime = Integer.parseInt(args[0]);
                if (map.getAreas().createArea(map.getSecLoc1(), map.getSecLoc2(), repTime)) {
                    sender.sendMessage(map.getAreas().infoLast());
                } else {
