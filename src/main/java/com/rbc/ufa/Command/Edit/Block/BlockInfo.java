...
public class BlockInfo extends UFACommandToDo {
    private final MapManager mapManager;

    public BlockInfo(UFACommandToDo superCmd, ModeManager modeManager, Message message) {
    ...
    public void toDo(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args.length) {
            case 0:
                sender.sendMessage(message.getMap("info.block_tip"));
            ...
            case 1:
                if (args[0].equals("loc")) {
                    sender.sendMessage(message.getMap("info.block_tip"));
                    for (String info : map.getBlocks().getLoc(map.getSecLoc1())) {
                        sender.sendMessage(info);
                    }
                } else {
                    try {
