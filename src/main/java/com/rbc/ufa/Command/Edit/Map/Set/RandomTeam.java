...
public class RandomTeam extends UFACommandToDo {
    private final MapManager mapManager;

    public RandomTeam(UFACommandToDo superCmd, ModeManager modeManager, Message message) {
    ...
    public void toDo(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            map.getSets().setRandomTeam(Boolean.parseBoolean(args[0]));
